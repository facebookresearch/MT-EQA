...
class House3DUtils():
  ...
  def get_dist_to_target_object(self, pos):
    ...
    return self.move_multiplier * dist  # why mul by move_multiplier? for computing physical distance.
  ...
  def compute_room_targets_iou(self, fine_classes, pos=None):
    """
    fine_classes: list of categories for some room
    pos: [x, y, z, yaw] (corr)
    """
    if pos:
      self.env.reset(x=pos[0], y=pos[2], yaw=pos[3])
    ...
    for fine_class in fine_classes:
      c = self.semantic_classes[fine_class]
      mask = np.all(seg == c, axis=2)
      iou = self._compute_iou(mask)
