...
class HouseParse():
  def __init__(self, dataDir, objrenderPath):
    
    self.dataDir = dataDir
    csvFile = csv.reader(open(os.path.join(objrenderPath, 'metadata/ModelCategoryMapping.csv'), 'r'))
    ...
    for i in levelsToExplore:
      for j in range(len(data['levels'][i]['nodes'])):
        ...
        # Objects
          ...
          self.house_fine_classes.append(self.modelCategoryMapping[data['levels'][i]['nodes'][j]['modelId']]['fine_grained_class'])
  ...
  """
  return list of [(obj1, obj2, dist)], sorted by distance (long to short)
  """
    ...
    for i in objList:
      for j in objList:
        ...
        if i.name + 'x' + j.name in objDistances or j.name + 'x' + i.name in objDistancesHash:
          ...
          dist = self.getClosestDistance(i.meta, j.meta, axes, order)
  ...
  """
  get pairwise distance for all rooms
  TODO: change distance computation to be shortest-path
  """
  def getAllPairwiseRoomDistances(self, RmList, axes=[0, 2], order=2):
    rmDistances, rmDistancesHash = [], {}
    for i in RmList:
      for j in RmList:
        if len(i.name) == 0 or len(j.name) == 0:
          continue
        if i.name == j.name:
          continue
        if i.name[0] + 'x' + j.name[0] in rmDistances or j.name[0] + 'x' + i.name[0] in rmDistancesHash:
          continue
        else:
          dist = self.getClosestDistance(i.meta, j.meta, axes, order)
          rmDistances.append((i, j, dist))
          rmDistancesHash[i.name[0] + 'x' + j.name[0]] = True
    
    return sorted(rmDistances, key=lambda x: -x[2])
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "-houseId",
    help="SunCG House ID",
    default="922f1d1e06fbdcc341c54bb868ec7fa5")
  parser.add_argument(
    "-dataDir",
    help="Directory that has the SUNCG dataset",
    default="data/SUNCGdata")
  parser.add_argument(
    "-objrenderPath",
    help='House3D directory',
    default='House3D/House3D'
  )
  args = parser.parse_args()

  Hp = HouseParse(dataDir=args.dataDir, objrenderPath=args.objrenderPath)
