...
ROOM_RATIO_TRHESH = 1.4
DIST_LOW_THRESH, DIST_HIGH_THRESH = 0.2, 1.0
...
class objectEntity():
  ...
  def __init__(self, name, bbox, meta, obj_id, room_id, unique):
    if name in self.translations: 
      self.name = self.translations[name]
    else: 
      self.name = name
    self.fine_class = name
    self.bbox = bbox
    self.meta = meta
    self.type = 'object'
    self.id = obj_id
    self.room_id = room_id
    self.unique = unique  # if the object is unique in whole house
...
class Engine():
  ...
  templates = {
    ...
    'unique_object_closer_inroom': 'is the <OBJ> closer to the <OBJ> than to the <OBJ>?',
    'unique_object_farther_inroom': 'is the <OBJ> farther from the <OBJ> than to the <OBJ>?',
    # object color comparison
    'object_color_compare_inroom': 'does the <OBJ> have same color as the <OBJ> in the <ROOM>?',
    'unique_object_color_compare_inroom': 'does the <OBJ> have same color as the <OBJ>?',
    ...
    'unique_object_bigger_inroom': 'is the <OBJ> bigger than <OBJ>?',
    'unique_object_smaller_inroom': 'is the <OBJ> smaller than <OBJ>?',
  ...
  def cacheHouse(self, Hp):
    ...
    for i in self.house.rooms:
      ...
      for j in room.meta['nodes']:
        fine_class = self.house.objects['0_' + str(j)]['fine_class']
        assert self.house.house_fine_classes.count(fine_class) >= 1
        unique = True if self.house.house_fine_classes.count(fine_class) == 1 else False
        obj = objectEntity(
  ...
  def queryObjectDistCompare(self, ent):
    ...
    for i in ent['elements']:
      ...
      unique = all([_.unique for _ in i[:3]])
      if unique:
        template = 'unique_' + template
  ...
  def queryObjectColorCompare(self, ent):
    ...
    for obj1, color1, obj2, color2 in ent['elements']:
      ...
      if rm == 'inroom' and  obj1.unique and obj2.unique:
        template = 'unique_' + template
  ...
  def roomSizePair(self, ent):
    ...
    for i in range(num_rooms):
      for j in range(num_rooms):
        ...
        if room_i.name[0] == 'dining room' and room_j.name[0] == 'kitchen': continue  # as too confusing
        if room_j.name[0] == 'kitchen' and room_i.name[0] == 'dining room': continue  # as too confusing
  ...
  def questionObjectBuilder(self, template, q_ent, a_str, q_type=None):
    ...
    if q_type in ['object_dist_compare_inroom']:
      for ent in q_ent:
        ...
        bbox.append({'id': ent.id, 'type': ent.type, 'box': ent.bbox, 'name': ent.name, 'fine_class': ent.fine_class, 'room_id': ent.room_id, 'room_name': ent.rooms[0].name[0], 'unique': ent.unique, 'target': True})
      mat = {}
    ...
    if q_type in ['object_color_compare_inroom', 'object_color_compare_xroom']:
      if 'inroom' in template:
        for ent in q_ent:
          ...
          bbox.append({'id': ent.id, 'type': ent.type, 'box': ent.bbox, 'name': ent.name, 'fine_class': ent.fine_class, 'color': color, 'room_id': ent.room_id, 'room_name': ent.rooms[0].name[0], 'unique': ent.unique, 'target': True})
      else:
    ...
    if q_type in ['object_size_compare_inroom', 'object_size_compare_xroom']:
      if 'inroom' in template:
        for ent in q_ent:
          ...
          bbox.append({'id': ent.id, 'type': ent.type, 'box': ent.bbox, 'name': ent.name, 'fine_class': ent.fine_class, 'size': size, 'room_id': ent.room_id, 'room_name': ent.rooms[0].name[0], 'unique': ent.unique, 'target': True})
      else:
        q_str = self.q_str_builder.prepareStringForTwo(q_str, q_ent[0].name, q_ent[1].name,
                  q_ent[0].rooms[0].name[0], q_ent[1].rooms[0].name[0])
        for ent in q_ent:
...
if __name__ == "__main__":
  ...
  parser.add_argument('--outputJson', default='questions_from_engine_mt.json', help='output json file')
  ...
  # house_ids = ['89156ff26b05fc334a84ba19dd5a8379']  # TODO: remove
