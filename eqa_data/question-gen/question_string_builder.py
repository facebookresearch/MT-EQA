...
class QuestionStringBuilder():
  ...
  def prepareStringForTwo(self, template, obj1, obj2, room1, room2):
    obj1 = ' '.join(obj1.split('_'))
    room1 = ' '.join(room1.lower().split('_'))
    obj2 = ' '.join(obj2.split('_'))
    room2 = ' '.join(room2.lower().split('_'))
    template = template.replace('<OBJ1>', obj1)
    template = template.replace('<OBJ2>', obj2)
    template = template.replace('<ROOM1>', room1)
    return template.replace('<ROOM2>', room2)
  ...
  def replaceRoom(self, template, room):
    ...
    if '<ROOM>' in template:
      return template.replace('<ROOM>', room, 1)
    elif '<ROOM-plural>' in template:
      return template.replace('<ROOM-plural>', room + 's', 1)

  # def prepareStringForColorCompare(self, template, obj1, obj2, room1, room2=None):
  #   template = template.replace('<OBJ1>', obj1)
  #   template = template.replace('<OBJ2>', obj2)
  #   template = template.replace('<ROOM1>', room1)
  #   if room2 is not None: template = template.replace('<ROOM2>', room2)
  #   return template
  
  # def prepareStringForRoomDistCompare(self, template, room1, room2, room3):
  #   template = template.replace('<ROOM1>', room1)
  #   template = template.replace('<ROOM2>', room2)
  #   return template.replace('<ROOM3>', room3)
  
  # def prepareStringForRoomSizeCompare(self, template, room1, room2):
  #   template = template.replace('<ROOM1>', room1)
  #   return template.replace('<ROOM2>', room2)
